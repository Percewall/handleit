		...
		#print $sql."\n";
   		...
   		if ($row = $sth->fetch()){
			if ($row['Timestamp']+$timestamp > time()){
				$return = TRUE;
				if (!!preg_match('/Items/i',$table) AND preg_match('/Achievements/i',$table) AND !preg_match('/Classes/i',$table) AND !preg_match('/Races/i',$table) AND !preg_match('/Perks/i',$table) AND !preg_match('/Quests/i',$table) AND !preg_match('/wowhead/i',$table)){
