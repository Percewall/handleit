...
class Guild {
	...
	private $realm;
	private $guildData;
	private $fields = array('members','achievements','news');
	private $cache;
	...
	public function getData() {
   	...
   	/**
   	 * Get a list of the completed news by the guild
   	 * @param String $sort Define what the list should be sorted by: timestamp|id|name
   	 * @param String $sortFlag Can be asc|desc
   	 */
   	public function getFeed($sort=FALSE, $sortFlag='desc', $limit=25){
   		$feedNews = $this->guildData['news'];
   		//$achievements['feedNewsTimestamp'] = $this->guildData['achievements']['achievementsCompletedTimestamp'];
   		$id_list = '';
   		for ($i = 0; $i < $limit; $i++){
   			// Build the new array to return
   			$feed['feedNews'][$i]=$feedNews[$i];
		}
		
		foreach ($feed['feedNews'] as $key => $news) {
			if ($news['type'] == 'itemLoot') {
				$item = new Item($this->region, $news['itemId']);
				$feed['feedNews'][$key]['itemName'] = $item->getName();
				$feed['feedNews'][$key]['ItemTime'] = $this->utils->get_fuzzy_time($news['timestamp']);
			}
		}
		
   		if ($sort){
			return $this->sortFeed($feed, $sort, $sortFlag);
		}
   		return $feed;
   	}
   	 ...
   	 */
   	public function getMembers($sort=FALSE, $sortFlag='asc', $minLevel=100){
   		$members = $this->guildData['members'];
   		if ($sort){
		...
		foreach ($members as $key=>$member){
			if ($members[$key]['character']['level'] < $minLevel) unset($members[$key]);
   		}
	...
	private function sortAchievements($array,$sortkey,$sortFlag){		
   	...
   	private function sortFeed($array,$sortkey,$sortFlag){
   		$arraysize = count($array);
   		foreach(array_keys($array) as $key){
  			$temp_array[$key] = $array[$key][$sortkey];
  		}
